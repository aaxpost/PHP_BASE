<?php
	...
	function validationEmail($data) 
		{	
		if ((preg_match('#^[0-9a-zA-Z-._]{4,}@[a-z]+\.[a-z]{2,4}$#', $data) == 1)) {	
			return true;
			} else {
				return false;
			}
			...
			if ((validationLoginLenth($_POST['login'])==true) and
				...
				if ($_POST['password'] == $_POST['confirm']) {
					...
					if (empty($login_user)) {
						...
						$query = "INSERT INTO user (login, password, user_name, middle_names, surname, date, email, registration_date, country, user_status) VALUES ('$login', '$password', '$user_name', '$middle_names', '$surname', '$date', '$email', CURRENT_DATE(), '$country', 'user')";

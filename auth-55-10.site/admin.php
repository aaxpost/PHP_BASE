<?php
	...
	if(!empty($_SESSION['user_status']) and
	...
	if(!empty($_SESSION['id']) and $_SESSION['user_status'] == 'admin') {
		...
		//Если есть гет запрос баним пользователя
		if (isset($_GET['banned_id'])) {
			$id = $_GET['banned_id'];
		...
		echo "<table border=\"1\"><tr><th>Логин</th><th>Статус</th><th>Управление</th><th>Права</th><th>Бан</th></tr>";
		...
		foreach ($base_user as $data_user) {
				...
				echo "<td bgcolor=\"$bgcolor\"><a href=\"admin.php?banned_id=$user_id\">Забанить</a><br><br></td>";		
